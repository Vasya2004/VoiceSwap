...
## AI-чистка текста (по желанию)

По умолчанию диктовка полностью локальна. Дополнительно можно включить
AI-чистку: после распознавания текст отправляется на OpenAI-совместимый сервер,
который исправляет пунктуацию, грамматику и очевидные ошибки распознавания.
Функция выключена по умолчанию и работает только с вашим собственным ключом.

Настройка:

1. Получите ключ у любого OpenAI-совместимого провайдера (по умолчанию —
   Groq, бесплатный ключ на [console.groq.com](https://console.groq.com/keys);
   также подойдут OpenAI, DeepSeek, OpenCode Zen, OpenRouter).
2. Откройте `Настройки` → раздел `AI-чистка текста`, вставьте ключ и нажмите
   `Сохранить ключ`. Ключ хранится только в Связке ключей macOS.
3. При необходимости измените базовый URL и модель (по умолчанию —
   `https://api.groq.com/openai/v1` и `openai/gpt-oss-20b`), включите
   переключатель и нажмите `Сохранить`. Для Groq запрос настроен на низкую
   задержку. Кнопка `Проверить подключение` проверяет ключ, адрес и доступность
   выбранной модели.

Важно: продиктованный текст отправляется выбранному провайдеру, и бесплатные
тарифы некоторых провайдеров (например, OpenCode Zen) могут сохранять его для
улучшения модели — перед личной диктовкой проверьте политику данных. Если
сервер недоступен или ответил ошибкой, вставляется исходный локальный текст —
диктовка никогда не теряется.
